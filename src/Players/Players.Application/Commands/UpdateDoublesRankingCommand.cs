...
public class UpdateDoublesRankingCommand : UpdateRankingBase
{
	private UpdateDoublesRankingCommand(PlayerId playerId, Ranking ranking,
	...
	public static Result<ICommand> Create(Guid id, int rank, int points, DateTime date)
	{
		...
		return new UpdateDoublesRankingCommand(playerId, ranking.Value, rankingPoints.Value, date);
	}
}
